...
function extractTags(block) {
	const match = block.match(/"?tags"?\s*:\s*\[([\s\S]*?)\]/);
...
function parseFriendsConfig(content) {
	const listMatch = content.match(/export const friendsConfig: FriendLink\[\] = \[([\s\S]*?)\r?\n[\t ]*\];/);
	if (!listMatch) {
		throw new Error('未找到 friendsConfig 数组，请检查 src/config/friendsConfig.ts 的格式。');
	}

	// 兼容不同的缩进风格（1 tab 或 2 tab 缩进）和 CRLF/LF 换行
	const friendBlocks = [...listMatch[1].matchAll(/\{([\s\S]*?)\r?\n[\t ]+\},?/g)].map((item) => item[1]);
...
function updateFriendsConfig(repoRoot, data) {
	...
	const updatedContent = content.replace(
		/export const friendsConfig: FriendLink\[\] = \[[\s\S]*?\r?\n[\t ]*\];/,
